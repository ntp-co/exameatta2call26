...
(function () {
  ...
  const CONFIG = Object.freeze({
    /** Duração total do exame, em segundos (90 minutos). */
    ...
    /** A partir de quantos segundos restantes o cronómetro fica "em alerta". */
    TIMER_WARNING_SECONDS: 5 * 60,

    /** Número de mudanças de aba a partir do qual se realça a linha no
     *  painel da Comissão como potencial indício de consulta externa. */
  ...
  function createInitialState() {
    return {
      /** Vista actual: landing | consent | checking | blocked | exam | result | admin-gate | admin */
      view: 'landing',
      ...
      timeLeftSeconds: CONFIG.EXAM_DURATION_SECONDS,
  ...
  Views.consent = function () {
    ...
    [
      ...
      'Compreendo que a Comissão Técnica poderá registar indicadores técnicos da sessão (como mudanças de aba do navegador durante o exame) para efeitos de integridade do processo.',
  ...
  Views.exam = function () {
    ...
    const header = dom('div', { class: 'exam-head' }, [
      ...
      dom('div', { class: 'timer' + (state.timeLeftSeconds <= CONFIG.TIMER_WARNING_SECONDS ? ' low' : ''), id: 'timer-display' }, [formatDuration(state.timeLeftSeconds)]),
    ]);
    ...
    const sectionLabel = dom('div', { class: 'section-label' }, [`Secção ${section.id} · ${section.name} (${section.weight}%)`]);
    ...
    const previousButton = SecondaryButton('← Anterior', actionGoToPreviousQuestion);
    if (state.currentQuestionIndex === 0) previousButton.disabled = true;
    ...
    card.appendChild(dom('div', { class: 'btn-row' }, [previousButton, nextOrSubmitButton]));
    ...
    if (state.adminStorageUnavailable) {
      ...
      state.adminSubmissions.forEach((submission) => {
        ...
        table.appendChild(dom('tr', {}, [
          ...
          dom('td', {}, [dom('span', { class: `pill ${submission.approved ? 'approved' : 'reproved'}` }, [submission.approved ? 'Aprovado' : 'Reprovado'])]),
  ...
  /** Actualiza apenas o texto/estilo do cronómetro, sem re-renderizar tudo (chamado a cada segundo). */
  function updateTimerDisplayOnly() {
    const timerElement = document.getElementById('timer-display');
    if (!timerElement) return;
    timerElement.textContent = formatDuration(state.timeLeftSeconds);
    timerElement.className = 'timer' + (state.timeLeftSeconds <= CONFIG.TIMER_WARNING_SECONDS ? ' low' : '');
  }
  ...
  // Regista mudanças de aba apenas durante o exame — usado como indicador
  // de integridade no painel da Comissão (nunca bloqueia o candidato).
  document.addEventListener('visibilitychange', () => {
    if (state.view === 'exam' && document.hidden) {
      state.tabSwitches += 1;
    }
  });
  ...
  /** Prepara a ordem (embaralhada, mas reprodutível) de perguntas e opções, e arranca o cronómetro. */
  function actionBeginExamSession() {
    ...
    state.timeLeftSeconds = CONFIG.EXAM_DURATION_SECONDS;
    state.tabSwitches = 0;
    actionSetView('exam');
    startExamTimer();
  }
  ...
  function actionGoToPreviousQuestion() {
  ...
  function actionConfirmSubmit() {
    ...
    if (answeredCount < state.orderedQuestions.length) {
      const proceed = window.confirm(
        `Respondeu a ${answeredCount} de ${state.orderedQuestions.length} questões. Deseja submeter mesmo assim?`
      );
      if (!proceed) return;
    }
    actionSubmitExam();
  ...
  async function actionSubmitExam() {
    stopExamTimer();
    const timeUsedSec = CONFIG.EXAM_DURATION_SECONDS - state.timeLeftSeconds;
  ...
  const CSV_COLUMNS = [
    ...
    { header: 'Estado', get: (r) => (r.approved ? 'Aprovado' : 'Reprovado') },
